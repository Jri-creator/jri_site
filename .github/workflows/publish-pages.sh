        ...
        class JriRadioPlayer {
            ...
            updateDocumentTitle() {
                if (this.tracks.length > 0) {
                    ...
                    const title = this.isPlaying ? 
                        ` ${track.title} - Jri Radio` : 
                        `Paused - Jri Radio`;
                    
                    if (this.scrollEnabled && title.length > 15) {
                        this.startTitleScroll(title);
                    } else {
            ...
            startTitleScroll(title) {
                // Simple title scrolling implementation
...
main() {
    ...
    if [ "$(cat $FILECOUNT_FILE)" -gt 0 ]; then
        ...
        echo ""
        echo "Note: You'll need to set up audio file hosting separately as GitHub Pages"
        echo "cannot serve the .jlres3 files directly. Consider using a CDN or file hosting service."
    else
